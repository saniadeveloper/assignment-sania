...
export const ProductsParent = ({search}) => {
    ...
    const getApiData= async ()=>{
        const allProducts = await axios.get("http://localhost:8080/laptop")
      ...
      const result2 = await axios.get(
        "http://localhost:8080/buttonlist"
      );
                ...
                <Grid container spacing ={2} >
                {
                    categ.map((item)=>{
                        return(
                            ...
                            <Button onClick={() => setCatData(item)} color="inherit" variant='contained' fullWidth>{item.b1}</Button>
                            </Grid>
                            </>
                        )
                    })
